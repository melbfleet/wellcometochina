...
// ─── New Way to Travel Modal ─────────────────────────────────────────────────────
function NewWayToTravelModal({
  typeId,
  onClose,
  onSaved,
}: {
  ...
  const createMut = trpc.admin.createWayToTravel.useMutation();
  ...
  async function handleSave() {
    ...
    try {
      ...
      toast.success("Way to Travel created");
      onSaved(result.id);
      onClose();
    } catch (e: any) {
...
// ─── Main Page ────────────────────────────────────────────────────────────────
export default function AdminWaysToTravelByType() {
  const params = useParams<{ typeId: string }>();
  const typeId = parseInt(params.typeId ?? "0");
  const [, navigate] = useLocation();
  const [showModal, setShowModal] = useState(false);
  const [deleteConfirm, setDeleteConfirm] = useState<number | null>(null);
  const [draggedWayToTravelId, setDraggedWayToTravelId] = useState<number | null>(null);

  const { data: type } = trpc.admin.getWayToTravelType.useQuery({ id: typeId }, { enabled: !!typeId });
  const { data: waysToTravel = [], refetch, isLoading } = trpc.admin.listWaysToTravelByType.useQuery({ typeId }, { enabled: !!typeId });
  const deleteMut = trpc.admin.deleteWayToTravel.useMutation();
  const reorderMut = trpc.admin.reorderWayToTravel.useMutation();

  async function handleDelete(id: number) {
    try {
      await deleteMut.mutateAsync({ id });
      toast.success("Way to Travel deleted");
  ...
  async function handleWayToTravelDrop(targetId: number) {
    if (!draggedWayToTravelId || draggedWayToTravelId === targetId) {
      setDraggedWayToTravelId(null);
      return;
    }
    const fromIndex = waysToTravel.findIndex(e => e.id === draggedWayToTravelId);
    const toIndex = waysToTravel.findIndex(e => e.id === targetId);
    if (fromIndex < 0 || toIndex < 0) {
      setDraggedWayToTravelId(null);
      return;
    }
    const next = [...waysToTravel];
  ...
  return (
    <AdminLayout>
      <div style={{ padding: "32px" }}>
        {/* Breadcrumb */}
        <button
          onClick={() => navigate("/admin/ways-to-travel")}
          style={{
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "24px" }}>
          <div>
            ...
            <p style={{ fontSize: "13px", color: "#888", marginTop: "4px" }}>
              {waysToTravel.length} way to travel item{waysToTravel.length !== 1 ? "s" : ""} in this type
            </p>
          </div>
          <button
        ...
        {isLoading ? (
        ...
        ) : waysToTravel.length === 0 ? (
          <div style={{ textAlign: "center", padding: "48px", border: "1px dashed #ddd", color: "#aaa" }}>
            <p style={{ fontSize: "13px" }}>No way to travel items in this type yet.</p>
            <p style={{ fontSize: "12px", marginTop: "4px", color: "#ccc" }}>Click "New Way to Travel" to add one.</p>
          </div>
        ) : (
          <div style={{ background: "#fff", border: "1px solid #eee" }}>
            ...
            <div style={{ display: "flex", alignItems: "center", padding: "10px 20px", background: "#e8e8e8", borderBottom: "1px solid #d8d8d8" }}>
              ...
              <span style={{ flex: 1, fontSize: "11px", letterSpacing: "0.12em", textTransform: "uppercase", color: "#888" }}>Way to Travel Name</span>
            ...
            {waysToTravel.map((exp, idx) => (
              <div
                key={exp.id}
                draggable
                onDragStart={e => {
                  setDraggedWayToTravelId(exp.id);
                ...
                onDrop={e => {
                  e.preventDefault();
                  handleWayToTravelDrop(exp.id);
                }}
                onDragEnd={() => setDraggedWayToTravelId(null)}
                style={{
                  display: "flex", alignItems: "center", padding: "14px 20px",
                  background: idx % 2 === 0 ? "#f2f2f2" : "#e8e8e8",
                  borderBottom: "1px solid rgba(0,0,0,0.04)",
                  cursor: "grab",
                  opacity: draggedWayToTravelId === exp.id ? 0.55 : 1,
                  outline: draggedWayToTravelId === exp.id ? "2px solid #F5569B" : "none",
                  outlineOffset: -2,
                }}
              >
                {/* Drag handle */}
                ...
                {/* Name — click to edit */}
                <button
                  onClick={() => navigate(`/admin/ways-to-travel/edit/${exp.id}`)}
      ...
      {showModal && (
        <NewWayToTravelModal
          ...
          onSaved={id => navigate(`/admin/ways-to-travel/edit/${id}`)}
        />
      )}
    </AdminLayout>
  );
