...
function LabelTagInput({
  ...
  return (
    <div>
      ...
      <p style={{ fontSize: "12px", color: "#bbb", marginTop: "8px" }}>Click a tag to add it. Selected tags are used to match similar ways to travel.</p>
    </div>
  );
}
...
// ─── Main Page ────────────────────────────────────────────────────────────────
export default function AdminWayToTravelEdit() {
  ...
  // Queries
  const { data: exp } = trpc.admin.getWayToTravel.useQuery({ id: expId }, { enabled: !!expId });
  ...
  // Copies stay within the independent Way to Travel collection.
  const { data: allWayToTravelTypes = [] } = trpc.admin.listWayToTravelTypes.useQuery();

  // Mutations
  const updateMut = trpc.admin.updateWayToTravel.useMutation();
  const saveDetailsMut = trpc.admin.saveWayToTravelDetails.useMutation();
  const saveLabelsMut = trpc.admin.saveWayToTravelLabels.useMutation();
  const copyMut = trpc.admin.copyWayToTravel.useMutation();
  ...
  async function handleSave() {
    ...
    try {
      ...
      await saveLabelsMut.mutateAsync({ wayToTravelId: expId, labels });
      toast.success("Way to Travel saved");
    } catch (e: any) {
  ...
  const backUrl = exp ? `/admin/ways-to-travel/type/${(exp as any).typeId ?? ""}` : "/admin/ways-to-travel";
      ...
      <div style={{ padding: "32px", maxWidth: "768px", margin: "0 auto" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "32px" }}>
          <div>
            ...
            <p style={{ fontSize: "13px", color: "#888", marginTop: "4px" }}>Edit way to travel details</p>
          </div>
          <button
            onClick={handleSave}
            disabled={saving}
        ...
        <Section title="Basic Information">
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "16px" }}>
            <div style={{ gridColumn: "1 / -1" }}>
              <label style={labelStyle}>Way to Travel Name *</label>
              <input
                value={name}
                onChange={e => setName(e.target.value)}
                placeholder="Way to Travel name"
                style={inputStyle}
                onFocus={e => { e.target.style.borderColor = "#F5569B"; }}
                onBlur={e => { e.target.style.borderColor = "#ddd"; }}
              />
          ...
          <div style={{ marginTop: "16px" }}>
            ...
            <textarea
              ...
              rows={3}
              placeholder="Brief overview of this way to travel..."
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", paddingTop: "16px", borderTop: "1px solid #e8e8e8", gap: "16px" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
            ...
            >
              ...
              {allWayToTravelTypes.length > 0 && (
                <optgroup label="Way to Travel Types">
                  {allWayToTravelTypes.map((t: any) => (
            ...
            {copyTargetId && (
              <button
                onClick={async () => {
                  ...
                  const targetId = parseInt(copyTargetId.replace(/^type-/, ""));
                  setCopying(true);
                  try {
                    ...
                    navigate(`/admin/ways-to-travel/edit/${result.newId}`);
                  } catch (e: any) {
        ...
        {exp?.slug && (
          <div style={{ display: "flex", justifyContent: "flex-end", paddingTop: "12px" }}>
            <button
              onClick={() => window.open(`/ways-to-travel/preview/${exp.slug}`, '_blank')}
              style={{
                display: "flex", alignItems: "center", gap: "8px",
                padding: "12px 32px", fontSize: "12px", letterSpacing: "0.1em", textTransform: "uppercase",
                background: "#1a1a1a", color: "#fff", border: "none", cursor: "pointer",
