...
const experiences = [
  ...
  },
  {
    ...
    typeSlug: 'culinary-journey',
    citySlug: 'xian',
    description: 'Street food tasting tour in Xi\'an\'s vibrant Muslim Quarter. Sample dumplings, noodles, and local specialties.',
  ...
  return [
    {
      ...
      content: `This experience offers an authentic and immersive encounter with local culture and traditions. You'll have the opportunity to interact with experienced guides and local community members who are passionate about sharing their heritage.`,
      sortOrder: 1
    },
    {
...
cities.forEach(city => {
  ...
  sql += `INSERT INTO cities (name, slug, description, shortDescription, highlights, bestTime, region, accentColor, previewImageUrl, isPublished, createdAt, updatedAt) VALUES ('${city.name}', '${city.slug}', '${city.description}', '${city.shortDescription}', '${city.highlights}', '${city.bestTime}', '${city.region}', '${city.accentColor}', '${imageUrl}', true, NOW(), NOW());\n`;
...
experienceTypes.forEach(type => {
  sql += `INSERT INTO experienceTypes (name, slug, description, icon, sortOrder, isPublished, createdAt, updatedAt) VALUES ('${type.name}', '${type.slug}', '${type.description}', '${type.icon}', ${type.sortOrder}, true, NOW(), NOW());\n`;
...
experiences.forEach(exp => {
  sql += `INSERT INTO experiences (name, slug, description, shortDescription, duration, difficulty, groupSize, price, isPublished, createdAt, updatedAt) VALUES ('${exp.name}', '${exp.slug}', '${exp.description}', '${exp.shortDescription}', '${exp.duration}', '${exp.difficulty}', '${exp.groupSize}', '${exp.price}', true, NOW(), NOW());\n`;
  console.log(`✓ ${exp.name}`);
});

// 生成关联关系的 SQL（需要在插入后获取 ID）
...
experiences.forEach(exp => {
  ...
  details.forEach((detail, idx) => {
    const escapedContent = detail.content.replace(/'/g, "\\'");
    sql += `INSERT INTO experienceDetails (experienceId, title, content, sortOrder, createdAt, updatedAt) SELECT id, '${detail.title}', '${escapedContent}', ${detail.sortOrder}, NOW(), NOW() FROM experiences WHERE slug = '${exp.slug}' LIMIT 1;\n`;
  });
